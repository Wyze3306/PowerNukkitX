...
public class LoginHandler extends BedrockSessionPacketHandler {
    ...
    public void handle(LoginPacket pk) {
        ...
        try {
            ...
            MaliciousPacketTracker.flag(session.getAddress(), "LoginPacket: " + e.getMessage());
        ...
        try {
            ...
            MaliciousPacketTracker.flag(session.getAddress(), "Skin payload: " + e.getMessage());
        ...
        if (!server.isWhitelisted((info.getUsername()).toLowerCase(Locale.ENGLISH))) {
            ...
            session.close(server.getWhitelistMessage());
            return;
        }
